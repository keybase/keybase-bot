// @flow
import {keybaseExec, keybaseStatus, timeout} from '../utils'
...
class Service {
  ...
  async _killCustomService(): Promise<void> {
    ...
    while (true) {
      ...
      if (++i >= 100) {
        throw new Error("The service didn't finish shutting down in time")
      }
    }
  }
  ...
  async startupService(): Promise<void> {
    ...
    // TODO: as a quick hack until this tail issue fixed, we're going to watch for the service
    // file AND the existence of net.Listen in here.
    if (this.serviceLogFile) {
      ...
      while (true) {
        ...
        try {
          ...
          const text = await promisify(fs.readFile)(fileToWatchFor, {encoding: 'utf-8'})
          if (text.indexOf('net.Listen on unix:') !== -1) {
            break
          }
        } catch (e) {
          if (++i >= 100) {
    ...
    // TODO: temporary revert to old monitoring
    // until tail issue solved
