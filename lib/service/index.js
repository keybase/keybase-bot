...
class Service {
  ...
  async startupService(): Promise<void> {
    ...
    const child = spawn('keybase', args, {env: Object.assign({KEYBASE_DISABLE_TEAM_AUDITOR: 1}, process.env)})
    ...
    const lineReader = this.serviceLogFile
      ? new Tail(this.serviceLogFile, {fromBeginning: true})
      : readline.createInterface({input: child.stderr})

    console.log(`${this.homeDir} -- about to monitor file ${this.serviceLogFile}`)
    await timeout(1000)
    ...
    return new Promise((resolve, reject) => {
      ...
      lineReader.on('line', (line: string) => {
        if (!resolvedYet) {
          console.log(`${this.homeDir} -- LINE - ${line}`)
        }
        if (line.indexOf('net.Listen on unix:') !== -1) {
          resolvedYet = true
          console.log(`${this.homeDir} -- FOUND THE LINE!`)
          resolve()
        }
      })
      lineReader.on('error', (err: Error) => {
        console.log(`${this.homeDir} -- 5 (error) ${err}`)
        reject(err)
      })
    })
  }
