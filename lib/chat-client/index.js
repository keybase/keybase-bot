...
class Chat extends ClientBase {
   ...
   */
  async listConvsOnName(name: string, options?: ChatListConvsOnNameOptions): Promise<ChatConversation[]> {
    await this._guardInitialized()
   ...
   * })
   */
  async join(channel: ChatChannel): Promise<void> {
    await this._guardInitialized()
    const res = await this._runApiCommand({
      apiName: 'chat',
      method: 'join',
   ...
   * })
   */
  async leave(channel: ChatChannel): Promise<void> {
    await this._guardInitialized()
    const res = await this._runApiCommand({
      apiName: 'chat',
      method: 'leave',
   ...
   */
  async newConv(channel: ChatChannel): Promise<void> {
    await this._guardInitialized()
    const args = {
      channel,
    }
   ...
   * bot.chat.newConv(channel).then(() => console.log('conversation created'))
   */
  async deleteConv(channel: ChatChannel): Promise<void> {
    await this._guardInitialized()
    const args = {
      channel,
    }
   ...
   */
  async attach(channel: ChatChannel, filename: string, options: ChatAttachOptions): Promise<void> {
    await this._guardInitialized()
