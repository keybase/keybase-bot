...
var Chat = /** @class */ (function (_super) {
    ...
    Chat.prototype._chatListen = function (onMessage, onError, channel, options) {
        ...
        child.on('error', function (err) {
            _this._bot.debugLog("got listen error " + err.message, 'E');
        });
        child.on('exit', function () {
            _this._bot.debugLog("got listen exit", 'I');
        });
        child.on('close', function () {
            _this._bot.debugLog("got listen close", 'I');
        });
        child.on('disconnect', function () {
            _this._bot.debugLog("got listen disconnect", 'I');
        });
        var lineReaderStderr = readline_1.default.createInterface({ input: child.stderr });
        lineReaderStderr.on('line', function (line) {
            _this._bot.debugLog("stderr from listener: " + line, 'E');
        });
        var lineReaderStdout = readline_1.default.createInterface({ input: child.stdout });
        var onLine = function (line) {
            _this._bot.debugLog("stdout from listener: " + line);
            try {
