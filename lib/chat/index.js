...
class ChatBot extends BaseBot {
  ...
  async watchChannelForNewMessages(channel: ChatChannel, onMessage: OnMessage) {
    await this._guardInitialized()
    this._handleChatListenOutput(onMessage, channel)
  }
  ...
  async watchAllChannelsForNewMessages(onMessage: OnMessage) {
    await this._guardInitialized()
    this._handleChatListenOutput(onMessage)
  }

  _handleChatListenOutput(onMessage: OnMessage, channel?: ChatChannel) {
    keybaseExec(this.homeDir, ['chat', 'api-listen'], {
      onStdOut: (line: string) => {
        try {
          ...
          } else if ((channel && channel.name === messageObject.msg.channel.name) || !channel) {
