 ...
 */
export const list = async (options: ChatOptionsList): Promise<ChatConversation[]> => {
...
const handleGuard = fn = async (store: Store) => {
  try {
    await guardInitialized(store, fn)
  } catch (error) {
    console.log(error)
  }
})
// Wrap exported chat methods using withStore to provide the store as the last argument to each function.
export default (store: Store) => ({
  list: withStore(store)(handleGuard(list)),
  read: withStore(store)(() => read),
  send: withStore(store)(() => send),
  delete: withStore(store)(() => deleteMessage),
  watchChannelForNewMessages: withStore(store)(() => watchChannelForNewMessages),
  watchAllChannelsForNewMessages: withStore(store)(() => watchAllChannelsForNewMessages),
})
