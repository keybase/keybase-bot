...
/**
 * Returns the full JSON object from `keybase status --json`, without any modifications or case
 * changes.
 * @ignore
 * @param workingDir - the directory containing the binary, according to top level Bot
 * @param homeDir - The home directory of the service you want to fetch the status from.
 * @example
 * keybaseStatusFull('/my/dir').then(status => console.log(status.username))
 */
async function keybaseStatusFull(workingDir: string, homeDir?: string): Promise<Object> {
  const status = await keybaseExec(workingDir, homeDir, ['status', '--json'], {json: true})
  return status
}

export default keybaseStatus
export {keybaseStatusFull}
