// @flow
import type {AnyApi} from '../types.js'
 ...
 */
export const withStore = (store: Store, allGuards?: WaitingGuardsFn) => {
  return async (wrapped: (store: Store) => AnyApi) => {
    if (allGuards) {
...
type GuardFn = (store: Store) => Promise<void>
type WaitingGuardsFn = (store: Store) => Promise<void>
export const withGuards = (guards: Array<GuardFn>): WaitingGuardsFn => async (store: Store) => {
  try {
