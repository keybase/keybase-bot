...
import {keybaseStatusFull} from './utils/keybaseStatus'
...
class Bot {
  ...
  /**
   * Returns the full JSON reply of `keybase status -j` for your bot.
   * @memberof Bot
   * @example
   * bot.status().then(status => console.log(status))
   */
  async status(): Promise<Object> {
    const info = this.myInfo()
    if (info) {
      const status = await keybaseStatusFull(this._workingDir, info.homeDir)
      return status
    } else {
      throw new Error("Cannot call keybase.status() on a bot that isn't initialized")
    }
  }
