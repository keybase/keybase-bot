// @flow
import {keybaseExec, randomTempDir, keybaseServiceStartup, killPIDs} from '../utils'
...
class BaseBot {
  ...
  async init(username: string, paperkey: string, options?: InitOptions): Promise<void> {
    ...
    const servicePID = await keybaseServiceStartup(this.homeDir)
    this.spawnedPIDs.push(servicePID)
   ...
   * @memberof BaseBot
   */
  async _initWithExistingService(homeDir: string, options?: InitOptions) {
  ...
  async deinit(): Promise<void> {
    await killPIDs(this.spawnedPIDs)
  }
