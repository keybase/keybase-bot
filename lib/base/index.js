// @flow
import {keybaseExec, mkdir, rmdir, keybaseServiceStartup} from '../utils'
...
const getCurrentUsernameAndDevicename = async (store: Store): Promise<DeviceUsernamePair> => {
  ...
  const status = await keybaseExec(tempDir, ['status', '--json'], {json: true})
...
const _init = (store: Store) => {
  ...
  const init: Init = async (username, paperkey, options) => {
    ...
    const {tempDir} = store.getState()
    await mkdir(tempDir)
    await keybaseServiceStartup(tempDir)
    await keybaseExec(tempDir, ['oneshot', '--username', username], {stdinBuffer: paperkey})
