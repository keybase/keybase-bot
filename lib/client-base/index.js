...
class ClientBase {
  ...
  async _guardInitialized(): Promise<void> {
    // guardInitialized only needed when running existingService mode,
    // not paperkey mode, since the user can't accidentally log in/out
    if (this.initialized !== 'paperkey') {
      const status = await keybaseStatus(this._workingDir, this.homeDir)
